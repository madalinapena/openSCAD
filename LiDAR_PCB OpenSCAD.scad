...
latime_piesa_plastic=54;
inaltime_surub=4;
raza_surub=1.25;
lungime_nema11=28.2;
latime_nema11=28.2;
inaltime_nema11=31.5;
distanta_gauri_nema11=23;
raza_slip_ring=22;
raza_baza_slip_ring=11;

module nema11()
{
    difference() {
    // body
     translate([-latime_nema11/2, -lungime_nema11/2, 0])
            color("gray") cube([latime_nema11, lungime_nema11, inaltime_nema11]);
    // holes
            ...
            translate([distanta_gauri_nema11, -distanta_gauri_nema11, inaltime_nema11 - 2]) cylinder (h=inaltime_surub, r=raza_surub, center = true, $fn=100);
            translate([-distanta_gauri_nema11/2, distanta_gauri_nema11/2, inaltime_nema11 - 2])  cylinder (h=inaltime_surub, r=raza_surub, center = true, $fn=100);
            translate([-distanta_gauri_nema11/2, -distanta_gauri_nema11/2, inaltime_nema11 - 2]) cylinder (h=inaltime_surub, r=raza_surub, center = true, $fn=100);
    }
...
module piesa_plastic()
{
    difference(){
        ...
        // margins
            ...
            translate ([0, 52, -4]) cube([lungime_piesa_plastic, inaltime_piesa_plastic, 4]);
        }
        // motor screw holes
        ...
        // slip ring screw holes
            translate ([inaltime_flange + 2 + raza_slip_ring, latime_piesa_plastic/2, -1] + [inaltime_flange * sin(30), inaltime_flange * cos(30), 0]) cylinder (h=inaltime_surub, r=raza_surub_slip_ring);
            translate ([inaltime_flange + 2 + raza_slip_ring, latime_piesa_plastic/2, -1] + [inaltime_flange * sin(150), inaltime_flange * cos(150), 0]) cylinder (h=inaltime_surub, r=raza_surub_slip_ring);
            translate ([inaltime_flange + 2 + raza_slip_ring, latime_piesa_plastic/2, -1] + [inaltime_flange * sin(270), inaltime_flange * cos(270), 0]) cylinder (h=inaltime_surub, r=raza_surub_slip_ring);

        // slip ring hole
        translate ([inaltime_flange + 2 + raza_slip_ring, latime_piesa_plastic/2, -1]) cylinder (h=inaltime_surub, r=raza_motor_hole);
        // IR hole
        translate([lungime_piesa_plastic - 3 - 4.5, latime_piesa_plastic/2 - 6, 0]) cube ([7, raza_slip_ring - 5, 5]);
        // ir screw hole
        translate([lungime_piesa_plastic - 4, latime_piesa_plastic/2 -9, -1]) cylinder (h=inaltime_surub, r=raza_surub, $fn = 10);
    }
}
//---------------------------------------------------
module slip_ring()
{
    ...
    // rotor
    translate([-4, 0, 14]) cylinder (h = 20, r = 2, $fn=100);  
}    
//---------------------------------------------------
module IR()
{
    difference(){
        color("green") cube([8, 23, 1], true);
        // screw hole
...
nema11();
translate ([41.5, 0, 16]) slip_ring();
translate ([71.5, 0, 34]) IR();
translate([0, -latime_piesa_plastic/2, inaltime_nema11]) piesa_plastic();
translate([0, 0, 35])roata_dintata();
translate([41, 0, 35])roata_dintata2();
translate([41, 5, 75])rotate ([90, 0, 0])piesa_plastic2();
translate([21, 11, 39.5])rotate([90, 0, 0])curea();
translate([20, -11, 39.5])rotate([90, 0, 0])curea2();
